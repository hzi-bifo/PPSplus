...
"""
    ...
    Interface to access the NCBI taxonomy in the sqlite3 format.
"""
...
class TaxonomyNcbi():
    ...
    def __init__(self, databaseFile, allowedRanks=TAXONOMIC_RANKS, considerNoRank=False):
        ...
        try:
            ...
            sys.stderr.write('TaxonomyNcbi: Failed to create connection to database: %s\n' % databaseFile)
            raise
    ...
    def getScientificName(self, ncbid, checkRank=False):
        ...
        if len(result) == 1:
            ...
            sys.stderr.write('TaxonomyNcbi: Cannot find name for ncbi: %s' % ncbid)
            return None

    def getNcbid(self, scientificName, checkRank=False):
        """
            @return: ncbi taxon id or None
            @rtype: int
        """
        ...
        if len(result) == 1:
            ...
            if len(result) == 0:
                sys.stderr.write('TaxonomyNcbi: Cannot find scientific name "%s" in the database.\n' % scientificName)
            else:
                sys.stderr.write('TaxonomyNcbi: scientific name "%s" is ambiguous!\n' % scientificName)
            return None

    def getNcbid2(self, name, checkRank=False):
        """
            @deprecated: one should use only scientific names
            @param name: doesn`t have to be a scientific name
            @return: ncbi taxon id or None
            @rtype: int
        """
        self.cursor.execute(str('SELECT T.ncbi_taxon_id FROM taxon_name TN, taxon T '
                                + 'WHERE TN.name=? AND TN.taxon_id=T.taxon_id'), (name,))
        result = self.cursor.fetchall()
        if len(result) == 1:
            ncbid = int(result[0][0])
            if checkRank and (not self.isRankNcbidAllowed(ncbid)):
    ...
    def _getParentNcbid(self, taxonId):
        ...
        result = self.cursor.fetchall()
        if len(result) != 1:
            sys.stderr.write('TaxonomyNcbi: Cannot find parent for taxon_id: %s\n' % taxonId)
            return None
        return int(result[0][0])
    ...
    def _getRank(self, taxonId):
        ...
        result = self.cursor.fetchall()
        if len(result) != 1:
            sys.stderr.write('TaxonomyNcbi: Cannot find rank for taxon_id: %s\n' % taxonId)
            return None
...
def _test4():
    ...
    print taxonomy.getChildrenNcbids(83763)
