...
def ppsOut2ppOut(inFile, outFile, taxonomy, taxaRanks):
    outBuff = TabSepFileFunctions.OutFileBuffer(outFile)
    namesList = TabSepFileFunctions.getColumnAsList(inFile, entryModifyFunction=None, colNum=0, sep='\t', comment='#')
    ncbidsList = TabSepFileFunctions.getColumnAsList(inFile, entryModifyFunction=None, colNum=1, sep='\t', comment='#')
...
def ppOut2PPSout():
    ...
    out = TabSepFileFunctions.OutFileBuffer(outFile)
